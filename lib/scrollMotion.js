...
// Drives a continuous scroll-linked "camera" over a scene: given
// keyframes of { at, scale, x, y } (x/y are transform-origin percentages
// 0-100), returns { scale, transformOrigin } motion values that smoothly
// interpolate through every keyframe in order as `progress` advances —
// one continuous move through the shot list, not a series of hard cuts.
export function useCameraPath(progress, keyframes) {
  ...
  const scale = useTransform(progress, (v) => lerpKeyframes(v, ats, scales));
  const transformOrigin = useTransform(progress, (v) => {
    const x = lerpKeyframes(v, ats, xs);
    const y = lerpKeyframes(v, ats, ys);
    return `${x}% ${y}%`;
  });
  return { scale, transformOrigin };
}

// A discrete moment (button press, value pop) centered on progress point
// `at`, ramping 0->1->back through a narrow window — for UI feedback that
// should read as a single scripted beat, not a continuous drift.
export function useGate(progress, at, width = 0.04) {
  return useTransform(progress, (v) => lerpKeyframes(v, [at - width, at, at + width], [0, 1, 0]));
}
