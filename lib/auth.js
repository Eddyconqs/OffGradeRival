...
/* ============================================================
   Grade Arena accounts are "full name + password" only — no
   ...
   we derive a deterministic, invisible one from the name:
     "Alex Rivera"  ->  "alex-rivera@graderival.vercel.app"
   That makes the full name function as the account's real
   identifier, so it has to be unique system-wide — enforced by
   Supabase's own "already registered" check at signup. (The
   domain is the app's real infrastructure hostname, unrelated to
   the "Grade Arena" brand name — see nameToEmail below.)
   ============================================================ */
...
export class AuthConfigError extends Error {
  constructor() {
    super(
      "Grade Arena isn't connected to a backend yet. Add NEXT_PUBLIC_SUPABASE_URL and NEXT_PUBLIC_SUPABASE_ANON_KEY to .env.local and restart the dev server."
    );
